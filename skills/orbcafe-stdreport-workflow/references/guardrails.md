...
- `useStandardReport` defaults (>= 1.0.6):
  ...
  - `-1` means ALL
- When using `CTable` standalone (without `useStandardReport`), pagination must be controlled via the `pagination` prop (including `page`, `pageSize`, `total`, `onPageChange`, `onPageSizeChange`).
